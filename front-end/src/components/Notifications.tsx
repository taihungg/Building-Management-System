...
export function Notifications() {
  ...
  const fetchBuildings = useCallback(async (keyword: string) => {
    ...
    try {
      const response = await fetch(`https://building-management-system.fly.dev/api/v1/buildings/dropdown?keyword=${encodeURIComponent(keyword ?? '')}`, {
        method: 'GET',
        headers: {
  ...
  const handleCreateAnnouncement = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    void (async () => {
      ...
      try {
        const response = await fetch('https://building-management-system.fly.dev/api/announcements', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'ngrok-skip-browser-warning': 'true'
